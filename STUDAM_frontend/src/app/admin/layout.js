...
export default function AdminLayout({ children }) {
    ...
    const menuItems = [
        ...
        { icon: '👨‍🏫', label: 'Enseignants', href: '/admin/users/teachers' },
        { icon: '🏢', label: 'Départements', href: '/admin/departments' },
        { icon: '📚', label: 'Cours', href: '/admin/courses' },
        { icon: '📝', label: 'Rapports', href: '/admin/reports' },
        { icon: '⚙️', label: 'Paramètres', href: '/admin/system' },
    ];
    ...
    useEffect(() => {
        ...
        setRecentActivity([
            { id: 1, type: 'user_created', description: 'Nouvel enseignant créé', timestamp: '2025-01-03 14:30', icon: '👤' },
            { id: 2, type: 'department_updated', description: 'Département Informatique mis à jour', timestamp: '2025-01-03 12:15', icon: '🏢' },
            { id: 3, type: 'system_backup', description: 'Sauvegarde automatique', timestamp: '2025-01-03 08:00', icon: '💾' },
        ]);
            ...
            <aside className={`fixed left-0 top-0 h-screen bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 border-r border-slate-700/50 z-30 transition-all duration-300 ${isSidebarOpen ? 'w-64' : 'w-20'}`}>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-slate-700/50">
                    <div className="bg-gradient-to-br from-violet-600/20 to-purple-600/20 rounded-xl p-4 border border-violet-500/30">
                        {isSidebarOpen ? (
                            <>
                                <p className="text-xs text-violet-300 font-medium mb-1">Version 1.0.0</p>
                                <p className="text-xs text-slate-400">STUDAM © 2025</p>
                            </>
                        ) : (
                            <p className="text-xs text-violet-300 text-center">V1.0</p>
                        )}
                    </div>
                </div>
            </aside>
            ...
            <div className={`transition-all duration-300 ${isSidebarOpen ? 'ml-64' : 'ml-20'}`}>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-slate-500 mt-1">Voici un aperçu de votre système</p>
                                </div>
                            </div>
                                    ...
                                    {showNotifications && (
                                        <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-slate-200 py-2 z-50">
                                            ...
                                            <div className="px-4 py-2 border-t border-slate-100">
                                                <Link href="/admin/notifications" className="text-sm text-violet-600 hover:text-violet-700 font-medium">
                                                    Voir toutes →
                                                </Link>
                                            </div>
                                        </div>
                                ...
                                <div className="relative">
                                    ...
                                    {showProfileMenu && (
                                        <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-200 py-2 z-50">
                                            ...
                                            <Link href="/admin/system" className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition-colors">
                                                <svg className="w-5 h-5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <footer className="bg-white border-t border-slate-200 py-4 px-6">
                    <div className="flex items-center justify-between text-sm text-slate-500">
                        <div>
                            © 2025 STUDAM. Tous droits réservés.
                        </div>
                        <div className="flex items-center gap-4">
                            <Link href="/privacy" className="hover:text-violet-600">Politique de confidentialité</Link>
